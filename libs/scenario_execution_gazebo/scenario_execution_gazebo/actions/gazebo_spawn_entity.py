...
class GazeboSpawnEntity(RunProcess):
    """
    Class to spawn an entity into simulation

    """

    def __init__(self, xacro_arguments: list, model: str):
        """
        init
        """
        super().__init__()
    ...
    def execute(self, entity_name: str, spawn_pose: list, world_name: str):  # pylint: disable=arguments-differ
    ...
    def on_executed(self):
        ...
        """
        self.feedback_message = "Executed spawning, waiting for response..."  # pylint: disable= attribute-defined-outside-init
    ...
    def shutdown(self):
        ...
        subprocess.run(["gz", "service", "-s", f"/world/{self.world_name}/remove",  # pylint: disable=subprocess-run-check
    ...
    def set_command(self, command):
        ...
        req = f'pose: {pose} name: "{self.entity_name}" allow_renaming: false sdf: "{command}"'
        super().set_command(["gz", "service", "-s", f"/world/{self.world_name}/create",
                             "--reqtype", "gz.msgs.EntityFactory",
                             "--reptype", "gz.msgs.Boolean",
                             "--timeout", "30000", "--req", req])
